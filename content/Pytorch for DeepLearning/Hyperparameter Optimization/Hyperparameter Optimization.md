...
The number of layers of a network defines the depth of it. Shallow networks are easier to interpret and computationally efficient, but the might struggle with complex datasets and often requires feature engineering.

Deeper networks can extract more complex features but involve more training parameters, longer training times and a higher risk of overfitting, particularly with small datasets. 

![[shallow_deep_nn.png]]

Numeber of neuros per layer:
More neurons can lead to better results with complex datasets, but also mean more weights to learn and so higher computational costs. For convolutional neural networks you can also specify the number of filters. the filter size and the padding.

![[Pasted image 20260918132125.png]]
